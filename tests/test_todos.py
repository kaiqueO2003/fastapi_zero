...
async def test_list_todos_filter_title_should_return_5_todos(
    ...
    session.add_all(
        TodoFactory.create_batch(
            5, user_id=user.id, title='Test todo 1'
        )
    )
    await session.commit()
...
@pytest.mark.asyncio
async def test_list_todos_filter_state_should_return_5(
    session, user, client, token
):
    expected_todos = 5
    session.add_all(
...
async def test_list_todos_filter_combined_should_return_5_todos(
    ...
    response = client.get(
        '/todos/?title='
        'est todo combined&description=combined&state=done',
        headers={'Authorization': f'Bearer {token}'},
    )
...
@pytest.mark.asyncio
async def test_list_todos_should_return_all_expecte(
